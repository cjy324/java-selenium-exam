...
public class Main {
	...
	private static void underKgSiteArticleCrawling() {
		...
		List<WebElement> elements = driver.findElements(By.cssSelector(".board_list tbody tr:not(.notice)"));
		...
		for (WebElement element : elements) {
			int no = Integer.parseInt(element.findElement(By.cssSelector(".no")).getText());
			String title = element.findElement(By.cssSelector(".title > a:first-child")).getText().trim();
			String writer = element.findElement(By.cssSelector(".author  > a:first-child")).getText().trim();
			String dateTime = element.findElement(By.cssSelector(".time")).getText().trim();
			int hit = Integer.parseInt(element.findElement(By.cssSelector(".readNum")).getText());
			int recommand = Integer.parseInt(element.findElement(By.cssSelector(".voteNum")).getText());

			Article article = new Article(no, title, writer, dateTime, hit, recommand);
			System.out.printf("%s / %s / %s / %s / %d /%d \n", article.getNo(), article.getTitle(), article.getWriter(),
					article.getDateTime(), article.getHit(), article.getRecommand());
		}
